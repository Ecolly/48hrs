...
class Map:
    ...
    def create_item(self, name, grid_items):
        ...
        if name == "Knife":
            return Weapon(name, grid_items, sprite_locs = 0, damage=4, durability=100, description="A common household chef's knife.", price=10)
        elif name == "Machete":
            return Weapon(name, grid_items, sprite_locs = 1, damage=5, durability=100, description="A long-bladed knife useful for cutting plants.", price=15)
        elif name == "Scimitar":
            return Weapon(name, grid_items, sprite_locs = 2, damage=7, durability=100, description="A sharp, curved blade.", price=20)
        elif name == "Sickle":
            return Weapon(name, grid_items, sprite_locs = 4, damage=3, durability=100, description="A crescent-shaped blade that can reach three enemies at once.", price=25)
        elif name == "Rapier":
            return Weapon(name, grid_items, sprite_locs = 5, damage=6, durability=100, description="A thin, slender weapon that can reach two tiles in front.",price=30)
        elif name == "Stick":
            return Weapon(name, grid_items, sprite_locs = 6, damage=2, durability=100, description="A thick tree branch that can be used as a crude weapon.",price=3)
        elif name == "Fury Cutter":
            return Weapon(name, grid_items, sprite_locs = 9, damage=20, durability=100, description="Ominous energy seeps off the cutting edge. The weapon returns 1/4 of damage dealt to the bearer.",price=50) #deducts 1/4 of attack damage from your hp
        elif name == "Windsword":
            return Weapon(name, grid_items, sprite_locs = 10, damage=9, durability=100, description="Imbued with magical runes, this longsword is more powerful than the average weapon.",price=80)
            ...
            return Staff(name, grid_items, reverse="Staff of Division",sprite_locs = fakenames_staffs_key[0], damage=10, projectile=True, description="Multiplies the target's HP by 2.", charges=5, rarity=2) #multiplies enemy's hp by 2
        elif name == "Staff of Division":
            return Staff(name, grid_items, reverse="Greater Healing Staff",sprite_locs = fakenames_staffs_key[1], damage=10, projectile=True, description="Divides the target's HP by 2.", charges=5, rarity=2) #divides enemy's hp by 2
        elif name == "Staff of Swapping":
            return Staff(name, grid_items, reverse="Staff of Warping", sprite_locs = fakenames_staffs_key[2], damage=10, projectile=True, description="Swap places with target.", charges=12, rarity=1) #swap places
            ...
            return Staff(name, grid_items, reverse="Staff of Alchemy", sprite_locs = fakenames_staffs_key[19], damage=10, projectile=True, description="The projectile materializes water over tiles it travels over.", charges=5, rarity=2) 
        # elif name == "Blasting Staff":
        #     return Staff(name, grid_items, reverse="Staff of Suffocation",sprite_locs = fakenames_staffs_key[20], damage=10, projectile=True, description="The projectile can be used to make tunnels.", charges=5, rarity=3)
        # elif name == "Staff of Suffocation":
        #     return Staff(name, grid_items, reverse="Blasting Staff", sprite_locs = fakenames_staffs_key[21], damage=10, projectile=True, description="The projectile materializes a 3x3 square of walls wherever it hits.", charges=5, rarity=2) 
            ...
            return Tome(name, grid_items, reverse="Tome of Injury", sprite_locs = fakenames_tomes_key[0], damage=10, projectile=False, description="Heal 15 HP from all creatures on the floor.", price=20)
        elif name == "Tome of Injury":
            return Tome(name, grid_items, reverse="Tome of Recovery", sprite_locs = fakenames_tomes_key[1], damage=10, projectile=False, description="Deduct 15 HP from all creatures on the floor.", price=20)
        elif name == "Tome of Promotion":
            return Tome(name, grid_items, reverse="Tome of Demotion", sprite_locs = fakenames_tomes_key[2], damage=10, projectile=False, description="Level up all creatures on the floor.", price=30)
        elif name == "Tome of Demotion":
            return Tome(name, grid_items, reverse="Tome of Promotion", sprite_locs = fakenames_tomes_key[3], damage=10, projectile=False, description="Level down all creatures on the floor.", price=30)
        elif name == "Immunity Tome":
            return Tome(name, grid_items, reverse="Paperskin Tome", sprite_locs = fakenames_tomes_key[4], damage=10, projectile=False, description="Temporarily boost defense of all creatures on the floor to 100.", price=40)
        elif name == "Paperskin Tome":
            return Tome(name, grid_items, reverse="Immunity Tome", sprite_locs = fakenames_tomes_key[5], damage=10, projectile=False, description="Makes all creatures on the floor die to a single physical attack.", price=40)
        elif name == "Sharpening Tome":
            return Tome(name, grid_items, reverse="Fortifying Tome", sprite_locs = fakenames_tomes_key[6], damage=10, projectile=False, description="Finds the last weapon in your inventory and boosts attack by 1.", price=20)
        elif name == "Fortifying Tome":
            return Tome(name, grid_items, reverse="Sharpening Tome", sprite_locs = fakenames_tomes_key[7], damage=10, projectile=False, description="Finds the last shield in your inventory and boosts defense by 1.", price=20)
        ...
        #    return Tome(name, grid_items, reverse="Tome of Identification",sprite_locs = fakenames_tomes_key[17], damage=10, projectile=False, description="Removes all identifications.",price=30)
            ...
            return Flask(name, grid_items, reverse="Cureall Flask", evaporation_rate=0.4, product="Water", liquid="Acid", sprite_locs = 1, description="Bubbling acid, this liquid deals damage to any who step in it.", price=15)
        elif name == "Cureall Flask":
            return Flask(name, grid_items, reverse="Acid", evaporation_rate=0.4, product="Water", liquid="Cureall", sprite_locs = 10, description="This liquid heals creatures, but has paralyzing side effect.", price=50)
            ...
            return Shield(name, grid_items, sprite_locs=1, defense=5, description="A sturdy shield painted with the emblem of a government.", price=12)
        elif name == "Mirror Shield":
            return Shield(name, grid_items, sprite_locs=2, defense=1, description="This shield is weak but will reflect projectiles.", price=25)
        elif name == "Wood Shield":
            return Shield(name, grid_items, sprite_locs=3, defense=3, description="A crude wooden shield usually used for training.", price=8)
        elif name == "Steel Shield":
            return Shield(name, grid_items, sprite_locs=4, defense=7, description="A thick, heavy shield made from a tough alloy.", price=25)
        elif name == "Armor Plate":
            return Shield(name, grid_items, sprite_locs=5, defense=18, description="Industrial plating once used to shield a tank from artillery fire. Holding it prevents weapons from being used.", price=50) #prevents weapons from adding to strength
        elif name == "Leaf Shield":
            return Shield(name, grid_items, sprite_locs=6, defense=1, description="A large, rubbery leaf.", price=2)
        elif name == "Spiked Shield":
            return Shield(name, grid_items, sprite_locs=0, defense=3, description="Studded with thorns, this shield will return damage back to any attackers.", price=30)
        elif name == "Sun Shield":
            return Shield(name, grid_items, sprite_locs=8, defense=4, description="Fractal networks of microcrystalline fiber adsorbs the effects of magical damage.", price=100)
        elif name == "Balance Shield":
            return Shield(name, grid_items, sprite_locs=7, defense=4, description="Shares damage between you and your attacker.", price=100)

        elif name == "Rock":
            return Miscellanious(name, grid_items, sprite_locs = 0, description = "A rock.", price=2)
        elif name == "Note":
            return Miscellanious(name, grid_items, sprite_locs = 1, description = "[REDACTED].", price=80)
        
        elif name == "3 Gold":
            return Miscellanious(name, grid_items, sprite_locs = 4, description = "A few coins.", price=3)
        elif name == "15 Gold":
            return Miscellanious(name, grid_items, sprite_locs = 5, description = "A pile of coins.", price=15)
